...
app.get('/api/historial', (req, res) => {
  const { nodo, desde, hasta } = req.query;

  if (!nodo) {
    return res.status(400).json({ error: "Debes enviar ?nodo=NODE1" });
  }

  let desdeMs = desde ? new Date(desde).getTime() : 0;
  let hastaMs = hasta ? new Date(hasta).getTime() : Date.now();
  ...
  try {
    ...
    for (const l of lineas) {
      ...
      // ==== FILTRO POR NODO ====
      if (nodoL !== nodo) continue;

      const fechaTotal = new Date(`${fecha} ${hora}`);
      const t = fechaTotal.getTime();
      if (t < desdeMs || t > hastaMs) continue;

      // ==== PARSEO CORRECTO DE VARIABLES ====
      const Bat = parseFloat((c[3] || "").replace("Bat=", "")) || null;
      const Temp = parseFloat((c[4] || "").replace("Tmp=", "")) || null;
      const pH   = parseFloat((c[5] || "").replace("pH=", "")) || null;
      const EC   = parseFloat((c[6] || "").replace("EC=", "")) || null;
      const DO   = parseFloat((c[7] || "").replace("DO=", "")) || null;

      resultados.push({
        timestamp: fechaTotal,
        Bat,
        Temp,
        pH,
        EC,
